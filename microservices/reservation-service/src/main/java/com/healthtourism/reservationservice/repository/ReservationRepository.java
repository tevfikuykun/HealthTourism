...
public interface ReservationRepository extends JpaRepository<Reservation, Long> {
    ...
    @Query("SELECT r FROM Reservation r WHERE r.doctorId = :doctorId AND r.appointmentDate BETWEEN :startDate AND :endDate AND r.status != 'CANCELLED'")
    List<Reservation> findConflictingReservations(@Param("doctorId") Long doctorId, @Param("startDate") LocalDateTime startDate, @Param("endDate") LocalDateTime endDate);

package com.healthtourism.accommodationservice.repository;
 ...
 * 
 * Note: This microservice version uses hospitalId (Long) instead of Hospital entity,
 * so JOIN FETCH is not applicable here, but pagination and specifications are still critical.
 */
@Repository
public interface AccommodationRepository extends JpaRepository<Accommodation, Long>, JpaSpecificationExecutor<Accommodation> {
     ...
     */
    @Query("SELECT a FROM Accommodation a WHERE a.hospitalId = :hospitalId AND a.isActive = true")
     ...
     */
    @Query("SELECT a FROM Accommodation a WHERE a.hospitalId = :hospitalId " +
           "AND a.isActive = true " +
    ...
    /**
     * @deprecated Use findNearbyAccommodations instead.
     */
    @Deprecated
    @Query("SELECT a FROM Accommodation a WHERE a.hospitalId = :hospitalId AND a.isActive = true ORDER BY a.distanceToHospital ASC")
     ...
     */
    @Deprecated
    @Query("SELECT a FROM Accommodation a WHERE a.hospitalId = :hospitalId AND a.isActive = true ORDER BY a.rating DESC")
